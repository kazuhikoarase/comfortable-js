        ...
        return {
            ...
            createEditor: function () {
                if (this.dataType == 'select-one') {
                    ...
                    return new comfortable.renderer.CheckBox(this);
                }
                return new comfortable.renderer.TextEditor(this);
            }
        };
    };
    ...
    (function (renderer) {
        renderer.createTextEditorSelectBox = function (editor) {
            ...
            var rollIndex = function (offset) {
                var index = Math.max(0, Math.min(optionsData.selectedIndex + offset, optionsData.options.length - 1));
            ...
            this.tables.forEach(function (table, i) {
                ...
                if (table.col == 1) {
                    table.left = -(renderParams.scrWidth > clientWidth ?
                        comfortable.util.translate(hViewPane.scrollLeft, 0, renderParams.scrWidth - clientWidth, ltRect.width, ltRect.width + renderParams.viewWidth - clientWidth, 'table.left') : ltRect.width);
                }
                if (table.row == 1) {
                    table.top = -(renderParams.scrHeight > clientHeight ?
                        comfortable.util.translate(vViewPane.scrollTop, 0, renderParams.scrHeight - clientHeight, ltRect.height, ltRect.height + renderParams.viewHeight - clientHeight, 'table.top') : ltRect.height);
                }
                if (table.col == 2) {
        ...
        ui.createOptions = function (optionsData) {
            ...
            var options = comfortable.util.extend(new comfortable.EventTargetImpl(), {
                ...
                rollIndex: function (offset) {
                    var index = Math.max(0, Math.min(selectedIndex + offset, optionsData.options.length - 1));
                    if (0 <= index && index < optionsData.options.length) {
                        selectedIndex = index;
                    }
                    else {
